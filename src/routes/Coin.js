        ...
        <div>
            <div className='coin-container'>
                ...
                <div className='rank'>
                     ...
                     <h1 class="head">{coin.name}</h1>
                </div>
                <div className='content'>
                    
                    <div className='info'>
                        <div className='coin-heading'>
                            {coin.image ? <img src={coin.image.small} alt='' className='iconn'/> : null}
                            < h6 >{coin.name}</h6><h6 class="subhead">d</h6>
                            {coin.symbol ? <h6>{coin.symbol.toUpperCase()}/USD</h6> : null}
                ...
                <div className='content'>
                    <div className='stats'>
                        <div className='left'>
                            <div className='row'>
                                <h4>24 Hour Low</h4>
                                {coin.market_data?.low_24h ? <p className='sub'>$ {coin.market_data.low_24h.usd.toLocaleString()}</p> : null}
                            </div>
                            <div className='row'>
                                <h4>24 Hour High</h4>
                                {coin.market_data?.high_24h ? <p className='sub'>$ {coin.market_data.high_24h.usd.toLocaleString()}</p> : null}                            </div>

                        </div>
                        <div className='right'>
                            <div className='row'>
                                <h4>Market Cap</h4>
                                {coin.market_data?.market_cap ? <p className='sub'>$ {coin.market_data.market_cap.usd.toLocaleString()}</p> : null}
                            </div>
                            <div className='row'>
                                <h4>Circulating Supply</h4>
                                {coin.market_data ? <p className='sub'>{coin.market_data.circulating_supply.toLocaleString()}</p> : null}
                            </div>
